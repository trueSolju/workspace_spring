...
//장바구니 상품 수정하는 코드
function setCnt(selectedTag,cartCode,itemPrice){
    ...
    .then((data) => {//data -> controller에서 리턴되는 데이터!
        ...
        checkAll(selectedTag.closest('tr').querySelector('.totalPrice-span').textContent= '₩'+totalPrice.toLocaleString());
        alert('상품의 수량이 변경 되었습니다.')
...
function deleteCarts(){
    ...
    const cartCode=[];
    for(const chk of chks){
        cartCode.push(chk.value);
    }


    location.href=`/cart/deleteCarts?cartCodeList=${cartCode}`;
}
function buys(){
    const chks=document.querySelectorAll('.chk:checked');
    if(chks.length==0){
